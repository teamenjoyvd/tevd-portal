...
- V8. Scope audit: run `git diff --stat HEAD` (covers staged + unstaged) AND `git status --short` or `git ls-files --others --exclude-standard` (covers untracked files, which the diff misses entirely); give a one-line justification per changed or new file tracing it to the GOAL or a logged DETOUR. Unjustifiable file -> revert it or flag it as unplanned. An empty result from both commands after edits this task is itself a FAIL — find where the changes went. Committing unexplained files is forbidden.
