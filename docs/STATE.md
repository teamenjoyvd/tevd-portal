...
**Remaining DoD gate — NOT done, static analysis only so far:** "normalized deep-equal response parity on every GET against local DEV (same data before/after)" — this needs a live authenticated run (Clerk session against hosted DEV) hitting every GET in the 17 routes pre/post-refactor and diffing bodies. Have not done this yet; per CLAUDE.md hard constraint ("NEVER mark Done on static analysis alone"), #603 is not Done until this runs. Next action: either script it (need a way to get an authenticated request — Clerk session cookie or a Playwright login flow) or do it via browser click-through.
