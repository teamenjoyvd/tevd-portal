...
## Now
Hooks code + tests done (12/12 vitest green); docs (GOTCHAS/BUILD/GCR/PROJECT/DEV_WORKFLOW) truthed up; PR template, runbook, and both workflows written (YAML parse-checked). Pending: commit; manual GitHub setup (`production` environment + reviewers, secrets `SUPABASE_ACCESS_TOKEN`/`SUPABASE_DB_PASSWORD_PROD`); human-run prod ledger audit per `docs/runbooks/PROD_MIGRATION_LEDGER_REPAIR.md` BEFORE approving any migrate-prod run; recommended follow-ups: GitHub ruleset on main, Vercel Preview env → DEV project.
