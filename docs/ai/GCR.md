...
7. **Post-merge tail (Done ≠ Merged):** after the PR merges — remove the issue's row from `docs/CLAIMS.md`; if the PR contained migrations, approve the pending `migrate-prod` run (`production` environment gate) and confirm it applied; smoke-check `https://www.teamenjoyvd.com`, then close the issue.
