...
| `/api/events/[id]/attend` | POST, DELETE | 2608-DEV-706 (D1 one-tap attend). POST body `{ share?: string }` — member self-registers via `lib/server/member-registration.ts` `attendEvent` (adopt-or-insert per D9, no self-attribution). Sends the member confirmation email on create/reactivate/adopt only (2608-DEV-707, template `member_event_confirmation`, language from the `tevd_lang` cookie); the response carries `emailed` so the client's success copy never claims an email that was skipped (no `contact_email`, over the shared 10/day recipient cap, or a send failure). DELETE soft-cancels via `cancelMemberRegistration`. 403 for `role === 'guest'`; single path segment under `/api/events/:id` so it stays Clerk-protected by the existing `lib/public-routes.ts` allowlist entry. A POST failure answers 400 with `{ error, code }` (2608-DEV-733), where `code` is `AttendFailureCode` — `guest_role \| not_found \| not_open \| event_ended \| event_full \| write_failed`. Clients select localized copy from `code`; `error` is English developer/log copy and must never be matched on. |
