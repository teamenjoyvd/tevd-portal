...
| `/api/admin/los-import` | POST | Imports LOS CSV via `import_los_members` RPC; reconciles sponsor changes |
...
| `import_los_members(rows jsonb)` | Bulk upsert LOS data; reconciles `profiles.upline_abo_number` + `tree_nodes` for any row where `sponsor_abo_number` changed |
