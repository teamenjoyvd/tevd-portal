...
export type VerifyAboProfile = {
  id: string
  first_name: string | null
  contact_email: string | null
}

export type VerifyAboOutcome = {
  status: number
...
/**
 * Validates a claimed ABO/upline pair against the LOS, then approves the request:
 * upsert_tree_node/approval RPC + member_notifications insert + member_event_log logging.
 * The RPC itself remains SECURITY DEFINER with its own internal auth check — this function
 * only relocates the calling orchestration, it does not change that boundary.
 */
export async function verifyAndApproveAbo(
  supabase: ServiceClient,
  userId: string,
  profile: VerifyAboProfile,
  claimed_abo: string,
  claimed_upline_abo: string
): Promise<VerifyAboOutcome> {
  // LOS existence check
  ...
  if (losMember.sponsor_abo_number !== claimed_upline_abo) {
    ...
    return {
      status: 422,
      body: { error: `Your sponsor ABO does not match the LOS record for ${claimed_abo}.`, error_code: 'upline_mismatch' },
    }
  }
  ...
  if (existingHolder) {
    ...
    return {
      status: 409,
      body: { error: `ABO ${claimed_abo} is already registered to another account.`, error_code: 'abo_already_claimed' },
    }
  }
  ...
  if (insertError || !requestRow) {
    return { status: 500, body: { error: insertError?.message ?? 'Failed to create request' } }
  }
  ...
  if (rpcError) {
    ...
    return { status: 500, body: { error: rpcError.message } }
  }
  ...
  return { status: 200, body: { status: 'approved', result } }
}
