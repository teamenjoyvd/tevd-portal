...
type ProfilePaymentRow = {
  ...
  trip: { id: string; title: string; destination: string; start_date: string; end_date: string; total_cost: number; currency: string } | null
  payments: unknown[]
}
...
function TripCard({
  ...
  return (
    ...
    >
      {/* Cover image — fixed 120px so card height is predictable */}
      {trip.image_url && (
      ...
      <div className="px-5 pt-4 pb-4 flex flex-col gap-3">
        ...
        {isRegistered && registrationStatus && (
          <div
            ...
            style={{ backgroundColor: STATUS_STYLES[registrationStatus].bg }}
          >
            <p className="text-sm font-medium" style={{ color: STATUS_STYLES[registrationStatus].color }}>
              {STATUS_STYLES[registrationStatus].label}
            </p>
            {registrationStatus === 'pending' && (
              <button
                ...
                style={{ color: STATUS_STYLES[registrationStatus].color }}
              >
                {t('trips.cancel')}
              </button>
            )}
...
export default function TripsPage() {
  ...
  // Single request replaces the previous N-waterfall per-trip registration fetches.
  // Returns all registrations + payments for the current user in one call.
  const { data: profilePayments = [] } = useQuery<ProfilePaymentRow[]>({
    queryKey: ['profile-payments'],
    queryFn: () => fetch('/api/profile/payments').then(r => r.json()),
    enabled: !!isSignedIn && !!profile?.id,
  })

  // Build a lookup: trip_id → ProfilePaymentRow
  ...
  const isLoading = tripsLoading || !isLoaded
