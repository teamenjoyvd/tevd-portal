...
// Pure functions — no React imports. Extracted from OrgChartCanvas.tsx per
// REFACTOR-018 to keep the canvas component focused on rendering.
...
export function layoutNode(node: LOSNode, cx: number, y: number, widthMap: Map<LOSNode, number>): LayoutNode {
  const children = node.children ?? []
...
export function collectEdges(root: LayoutNode): Array<{ x1: number; y1: number; x2: number; y2: number }> {
