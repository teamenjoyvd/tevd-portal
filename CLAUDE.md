...
3. Update Gotchas below if new non-obvious decisions were made.
...
**GATHER:** Read CLAUDE.md + CONTEXT.md sections relevant to the ticket. Read Target Files + DoD from Airtable. Probe vague DoD before writing code.
...
| Duplicate records | 22 ghost records exist — always filter `Duplicate = false/empty`. |
...
1. Read existing table/function first with `execute_sql`.
2. DDL → `apply_migration` only. Never raw `execute_sql` for DDL.
3. Save SQL to `supabase/migrations/YYYYMMDDNNNNNN_name.sql`.
4. Run `generate_typescript_types` → write to `types/supabase.ts` → commit in same push as migration.
5. `npx tsc --noEmit` → zero errors → commit.
...
### CI Type Check
On every push to `main`: (1) Supabase type drift check — diffs live DB against `types/supabase.ts`, fails with fix command if stale. (2) `tsc --noEmit`. Required secret: `SUPABASE_ACCESS_TOKEN` in repo secrets. Fix: `supabase gen types typescript --project-id ynykjpnetfwqzdnsgkkg > types/supabase.ts` then commit.

---

## 8. Reference

For schema, directory structure, design system, i18n, access control flows, and release history — read `docs/ai/CONTEXT.md`.
